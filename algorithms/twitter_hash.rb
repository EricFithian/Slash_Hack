...
#Find the RT count
# p ruby_hash['statuses'][3]['entities']['hashtags'][0]['indices']


#p ruby_hash['statuses']

#230 Mexican Heaven and Mexican Hell names
# p ruby_hash['statuses'][2]['text']
code = "Mexican Heaven"

ruby_hash.each do |status|
  if status.contains(code)
  else
  statuses.each_index do |second_level|
    if second_level.contains(code)
